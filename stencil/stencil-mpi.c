...
void do_cal7(cptr_t a0, ptr_t a1, cptr_t b0, ptr_t b1, cptr_t c0, ptr_t c1, int x, int y, int z, int ldx, int ldy) {
    ...
    for (int t = 0; t < nt; ++t) {
        ...
        if (grid_info->p_id != grid_info->p_num - 1) {
            MPI_Sendrecv((void *) &a0[INDEX(0, 0, z_end - 1, ldx, ldy)], ldx * ldy, MPI_DOUBLE, grid_info->p_id + 1, 0, \
                        (void *) &a0[INDEX(0, 0, z_end, ldx, ldy)], ldx * ldy, MPI_DOUBLE, grid_info->p_id + 1, 1,
                         MPI_COMM_WORLD, &status);
        }
        if (grid_info->p_id != 0) {
            MPI_Sendrecv((void *) &a0[INDEX(0, 0, z_start, ldx, ldy)], ldx * ldy, MPI_DOUBLE, grid_info->p_id - 1, 1, \
                        (void *) &a0[INDEX(0, 0, z_start - 1, ldx, ldy)], ldx * ldy, MPI_DOUBLE, grid_info->p_id - 1, 0,
                         MPI_COMM_WORLD, &status);
        }
        int bs_y = 16;
            ...
            for (int z = z_start; z < z_end; ++z) {
                for (int y = yy; y < yy + bs_y; ++y) {
                    for (int x = x_start; x < x_end; x += 4) {
                        do_cal7(a0, a1, b0, b1, c0, c1, x, y, z, ldx, ldy);
                    }
                }
            }
        }
